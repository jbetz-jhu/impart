...
# Load Data: Revert to 1400 days study time
...
# Determine Timing of 2nd Interim Analysis
data_t_1400 <-
  data_at_time_t(
...
data_t_1400_prepared <-
  prepare_monitored_study_data(
    data = data_t_1400$data,
    study_time = last_event,
    id_variable = ".id",
    covariates_variables = c("x_1", "x_2", "x_3", "x_4"),
...
# Plot/Count Outcomes
plot_outcome_counts(
  prepared_data = data_t_1400_prepared
)

count_outcomes_at_time_t(
  prepared_data = data_t_1400_prepared,
  study_times = c(700, 950, 1400)
)


# Compute Information Trajectory
data_t_1400_trajectory <-
...
example_1_ia_2 <- data_n_interim_2$data
...
usethis::use_data(example_1_ia_2, overwrite = TRUE)
